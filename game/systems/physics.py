...
from game.utils.constants import ARENA_PADDING, SCREEN_HEIGHT, SCREEN_WIDTH
...
class PhysicsSystem:
    ...
    def _clamp_tank(self, tank) -> None:
        """Keep tank within the playable arena."""
        tank.x = max(ARENA_PADDING, min(SCREEN_WIDTH - ARENA_PADDING, tank.x))
        tank.y = max(ARENA_PADDING, min(SCREEN_HEIGHT - ARENA_PADDING, tank.y))

    def _check_bullet_boundary(self, bullet) -> None:
        """Destroy bullet (or bounce, if supported) when it leaves the arena."""
        out_x = bullet.x < 0 or bullet.x > SCREEN_WIDTH
        out_y = bullet.y < 0 or bullet.y > SCREEN_HEIGHT
