...
class TestTankToTankCollision:
    """Verify tank-to-tank collisions push apart but apply no damage."""

    def test_collision_does_not_apply_damage(self):
        """Two overlapping tanks collide — neither takes damage."""
        cs = CollisionSystem()
        a = _StubTank2(x=0, y=0, angle=0.0, health=100, vx=100, vy=0)
        b = _StubTank2(x=5, y=0, angle=0.0, health=100, vx=-100, vy=0)
    ...
    def test_dead_tank_skipped_in_tank_vs_tank(self):
        ...
        b.is_alive = False
        cs._tanks_vs_tanks([a, b])
        assert a.health == 100  # a must not be damaged
